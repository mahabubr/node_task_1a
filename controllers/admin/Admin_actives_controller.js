...
app.get(
  ...
  async function (req, res, next) {
    ...
    const viewModel = new activesAdminAddViewModel(
      db.active,
      "Add active",
      "",
      "",
...
app.post(
  "/admin/actives-add",
  SessionService.verifySessionMiddleware(role, "admin"),
  ValidationService.validateInput({ name: "required", handle: "required" }),
  async function (req, res, next) {
    if (req.session.csrf === undefined) {
      req.session.csrf = SessionService.randomString(100);
    }
    ...
    try {
      ...
      const data = await db.active.insert({
        name,
        description,
        handle,
        variables_scores: JSON.stringify(rest),
      });
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      if (exists.variables_scores) {
        const currentOutputVariablesScores = JSON.parse(
          exists.variables_scores
        );
        Object.keys(currentOutputVariablesScores).forEach((varScore) => {
          viewModel.form_fields[varScore] =
            currentOutputVariablesScores[varScore];
        });
      }
...
app.post(
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      let data = await db.active.edit(
        { name, description, handle, variables_scores: JSON.stringify(rest) },
        id
      );
      if (!data) {
        viewModel.error = "Something went wrong";
    ...
    try {
      ...
      for (const product of data.products) {
        if (product.tags == "Personalized Cream" && product.title !== "Blank") {
          const activeExists = await db.active.getByFields({
            name: product.title,
          });
          if (activeExists) {
...
app.get(
  ...
  async function (req, res, next) {
    try {
      ...
      var viewModel = new activesAdminDetailViewModel(
        db.active,
        "Active details",
        "",
        "",
