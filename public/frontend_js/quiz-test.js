...
async function askQuestion(totalQuizQuestions, counter, fromBack) {
  ...
  if (type == 1) {
    ...
    ques.answers.forEach((val, index) => {
      if (val.answer) {
        // Add event listener to each option button to call checkAllergie function
    ...
    // Handle already answered questions
    if (alreadyAnswered && alreadyAnswered.answer) {
...
function checkAllergie(answer) {
  ...
  const clickVal = clickBtn.attr("data-val");

  const allergyVal = ["Banana", "Olive", "Sunflowers"];

  if (allergyVal.includes(clickVal)) {
    terminateQuiz();
  }
}

async function getTerminateData() {
  try {
    const response = await fetch(url_preset + "/api/v1/terminate");
    ...
    console.error("Error terminate data:", error);
    throw error;
  }
}

async function terminateQuiz() {
  const { data } = await getTerminateData();

  const blackOverlay = $("<div></div>").css({
    position: "fixed",
    top: 0,
    left: 0,
    width: "100%",
  ...
  const text = $("<div></div>").text(data?.title).css({
    color: "white",
    fontSize: "20px",
    marginTop: "0",
  });

  const text1 = $("<div></div>").text(data?.disclaimer).css({
    color: "white",
    fontSize: "26px",
    marginTop: "20px",
  });

  const text2 = $("<div></div>").text(data?.count).css({
    color: "white",
    fontSize: "26px",
    marginTop: "20px",
  });
  ...
  blackOverlay.append(textContainer);
  ...
  const countDownInterval = setInterval(() => {
    countDown--;
    text2.text(countDown);

    if (countDown <= 0) {
      clearInterval(countDownInterval);
      window.location.reload();
      window.location.href = "http://localhost:3001/quiz-test.html";
    }
  }, 1000);
}
